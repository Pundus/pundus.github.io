 ...
 * juegos guardados. Sin embargo, el tipo de artículo genérico en los parámetros 
 * de la receta no admite partidas guardadas.
 ...
 * - Los productos fallidos son elementos que el jugador recibe directamente en 
 *   el inventario de una nave fallida. Este es un parámetro opcional.
 * - Las herramientas son elementos necesarios para elaborar la receta, pero que 
 *   NO se consumen en la elaboración.
 ...
 * Las recetas pueden usar "tipos de elementos genéricos" para sus ingredientes o 
 * herramientas. Lo que esto significa es que si necesita una herramienta de 
 * "sartén" para elaborar una "tortilla", puede designar su "sartén de hierro" y 
 * su "sartén de acero" como un elemento de tipo "sartén", y luego cualquiera 
 * podría usarse para elaborar la "tortilla".
 *
 * Consulte la sección de etiquetas de notas para configurar el tipo de elemento.
 *
 * Precaución: Se debe tener cuidado si un artículo pertenece a varias categorías 
 * genéricas y la receta usa varias categorías genéricas, ya que el artículo podría 
 * contarse dos veces al calcular la cantidad de artículos que tiene el jugador.
 * --------------------------Etiquetas de notas------------------------------------
 ...
 * Descubrir: Descubre (o no descubre) la receta dada por su nombre
 * Escena de llamada: llama a la escena de elaboración. Especifique un tipo de 
 * profesión (distingue entre mayúsculas y minúsculas) para incluir solo ciertas 
 * recetas en la escena.
 * Descripción del conjunto: Cambia la descripción de la receta dada.
 * Reinicializar: Reinicializa los datos de elaboración como si hubieras comenzado 
 * un nuevo juego.
 ...
 *
 * Versión 1.0.1
 * - Se facilitó la selección de artículos/armaduras/armas para elaborar recetas.
...
CGMZ_Recipe.prototype.initialize = function(recipe) {
	...
	this._subcategory = recipe.Subcategory;
	this._quality = recipe.Quality;
...
CGMZ_Core.prototype.initializeRecipeData = function(reinitialize) {
	...
	for(let i = 0; i < CGMZ.Crafting.Recipes.length; i++) {
		...
		if(!existingRecipe) {
			...
			if(!existingRecipe._learnOnCraft) {
				existingRecipe._learnOnCraft = recipe._learnOnCraft;
			}
			if(!existingRecipe._quality) {
				existingRecipe._quality = recipe._quality;
			}
			if(!existingRecipe._subcategory) {
				existingRecipe._subcategory = recipe._subcategory;
			}
		}
	}
};
	...
	for(const section of CGMZ.Crafting.DisplayWindowInfo) {
		switch(section) {
			...
			case "Success Rate":
				...
				break;
			case "Quality":
				this.drawRecipeStandardLine(CGMZ.Crafting.QualityLabelText, recipe._quality);
				this._neededHeight += this.lineHeight();
				break;
			case "Subcategory":
				this.drawRecipeStandardLine(CGMZ.Crafting.SubcategoryLabelText, recipe._subcategory);
				this._neededHeight += this.lineHeight();
				break;
			case "Exp":
			...
			case "Current Level":
				if(Imported.CGMZ_Professions && $cgmz.getProfession(recipe._profession)) {
					this.drawRecipeStandardLine("Current " + recipe._profession + " " + CGMZ.Crafting.CurrentLevelText, $cgmz.getProfession(recipe._profession).getBuffedLevel());
					this._neededHeight += this.lineHeight();
				}
				break;
			case "Description":
				...
				break;
			case "Products":
				this.drawRecipeItems(recipe._products, true, CGMZ.Crafting.ShowProductAmount ); // This function handles needed height itself
				break;
			case "Tools":
				this.drawRecipeItems(recipe._tools, false, CGMZ.Crafting.ShowToolAmount ); // This function handles needed height itself
				break;
			case "Ingredients":
				this.drawRecipeItems(recipe._ingredients, false, CGMZ.Crafting.ShowIngredientAmount ); // This function handles needed height itself
				break;
			case "Fail Products":
				this.drawRecipeItems(recipe._failProducts, true, CGMZ.Crafting.ShowFailProductAmount ); // This function handles needed height itself
				break;
			case "Info Header":
