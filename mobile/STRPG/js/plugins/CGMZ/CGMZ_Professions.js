 ...
 * @default 0
 * @min -1
 * @desc Índice de iconos a utilizar para la profesión. Establecer en -1 para no usar.
 *
 * @param Picture
 * @text Imagen
 * @type file
...
CGMZ.Versions["Professions"] = "1.4.1";
CGMZ.Professions = {};
		...
		switch(infoType) {
			...
			case "Recipes Header":
				if(Imported.CGMZ_Crafting) {
					this.CGMZ_drawHeader(CGMZ.Professions.RecipeHeaderText, this._neededHeight);
					this._neededHeight += this.lineHeight();
				}
				break;
			case "Recipes":
...
CGMZ_Window_Toast.prototype.processCustomToast = function(toastObject) {
	...
	if(toastObject.hasOwnProperty('CGMZProfessionToast')) {
		var totalWidth = this.textWidth(toastObject.name + " " + CGMZ.Professions.LevelUpText);
		var spaceWidth = this.textWidth(" ");
		var nameTextWidth = this.textWidth(toastObject.name);
		var nameXOffset = (this.contents.width - totalWidth) / 2;
		this.drawText(toastObject.name, nameXOffset, 0, nameTextWidth, 'left');
		var spaceXOffset = nameXOffset + nameTextWidth;
		this.drawText(" ", spaceXOffset, 0, spaceWidth, 'left');
		var levelUpTextWidth = this.textWidth(CGMZ.Professions.LevelUpText);
		var levelUpTextXOffset = spaceXOffset + spaceWidth;
		this.changeTextColor(ColorManager.normalColor());
		this.drawText(CGMZ.Professions.LevelUpText, levelUpTextXOffset, 0, levelUpTextWidth, 'left');
